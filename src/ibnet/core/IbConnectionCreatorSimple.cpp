...
IbConnectionCreatorSimple::IbConnectionCreatorSimple(uint16_t ownNodeId,
        uint16_t qpMaxRecvReqs, uint16_t qpMaxSendReqs,
        std::shared_ptr<IbSharedRecvQueue> sharedRecvQueue,
        std::shared_ptr<IbCompQueue> sharedRecvCompQueue) :
    IbConnectionCreator(ownNodeId),
...
{
    auto ret = std::make_shared<IbConnection>(GetOwnNodeId(), connectionId,
        device, protDom);
